...
class NLVRImage:
    ...
    def determine_object_index(self, object):
        ...
        if object.color == Color.YELLOW:
            if object.shape == Shape.CIRCLE:
                return 1
            elif object.shape == Shape.SQUARE:
                return 2
            elif object.shape == Shape.TRIANGLE:
                return 3

        elif object.color == Color.BLACK:
            if object.shape == Shape.CIRCLE:
                return 4
            elif object.shape == Shape.SQUARE:
                return 5
    ...
    def png(self, box_size, filename):
        ...
        # list that is going to hold all the masks
        mask_list = []
        class_names_list = []
        first_object = True
        ...
        for i, box in enumerate(self.boxes):
            ...
            for obj in box:
                ...
                # for each object that is in the box, determine the mask
                # make the mask
                mask = Image.new('L', (int(box_size * len(self.boxes) + box_size * (len(self.boxes) - 1)/2), box_size))
                ...
                if obj.shape == Shape.CIRCLE:
                    ...
                    # draw mask
                    mask_draw.ellipse([x_start, y_start, x_end, y_end], fill=1)
                    class_names_list.append(self.determine_object_index(obj))

                elif obj.shape == Shape.SQUARE:
                    draw.rectangle([x_start, y_start, x_end, y_end], fill=obj.color.as_rgb())
                    # draw mask
                    mask_draw.rectangle([x_start, y_start, x_end, y_end], fill=1)
                    class_names_list.append(self.determine_object_index(obj))
                    ...
                    # draw mask
                    mask_draw.polygon([bottom_left, bottom_right, top], fill=1)
                    class_names_list.append(self.determine_object_index(obj))
                ...
                if not first_object:
                    ...
                    mask_list = np.concatenate((mask_list, mask_array), axis=2)
                else:
        ...
        mask_list = np.asarray(mask_list).astype(bool)

        # EXTRA: I have added this
        # reshape the masks to the shape that they will need to be for the mask_rcnn
        mask_list = img_as_bool(resize(mask_list, (64, 256)), force_copy=True)
        mask_list = mask_list.astype(np.uint8)
        try:
            # reshape the mask for the MASKRCNN
            mask_list = np.reshape(mask_list, (64, 256, -1))
        except:
            print(mask_list.shape)
            return
        class_names_list = np.asarray(class_names_list)
    
        img.save(filename + '.png', "PNG")
        np.save(filename + '_mask.npy', mask_list)
        np.save(filename + '_mask_index', class_names_list)
    
    def is_equal(self, other_image):
        return self.json() == other_image.json()        


def image_from_list(sox_list, image_type):
    new_boxes = []
    for box in box_list:
        new_box = []
        for obj in box:  
