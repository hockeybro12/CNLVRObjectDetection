...
from nlvr_image import NUM_BOXES, BOX_OBJECT_RATIO, NLVRObject, NLVRImage, ImageType, Shape, Color, Size
        ...
        while not found_split:
            ...
            for i in range(NUM_BOXES): 
                ...
                for j in range(num_objects):
                    ...
                    # determine where the object must be placed in the box
                    x_pos, y_pos, size = place_object(objects, size, image_type)

                    # generate the object and append it to the list of objects
                    # just has a bunch of helper functions for an object of this shape/color/size/pos
...
# Comment out the rest of this part if generating your own images

# change this if you are generating validation data. Set to True
generating_validation = False

if generating_validation:
    scatter_images_to_generate = 200
    tower_images_to_genreate = 100

else:
    scatter_images_to_generate = 3000
    tower_images_to_genreate = 1500

# create a json file with all the information
#750
image_path_information = []

for i in tqdm(range(0, scatter_images_to_generate, 2)):

    if generating_validation:
        filename = 'validation_generated_images/image_' + str(i)
    else:
        filename = 'generated_images/image_' + str(i)
    ...
    image_path_information.append(filename)
    ...
    if generating_validation:
        ...
        filename = 'generated_images/image_' + str(i)
    current_image = generate_image(ImageType.SCATTER, current_image)
...
# save the image path information
if generating_validation:
    np.save('validation_scatter_image_path', image_path_information)
else:
    np.save('scatter_image_path', image_path_information)
...
#750
tower_image_path_information = []

for i in tqdm(range(0, tower_images_to_genreate, 2)):

    if generating_validation:
        filename = 'validation_generated_images_tower/image_' + str(i)
    else:
        filename = 'generated_images_tower/image_' + str(i)
    
    current_image = generate_image(ImageType.TOWER)

    if not current_image:
        continue

    current_image.png(box_size=BOX_SIZE, filename=filename)
    tower_image_path_information.append(filename)
    ...
    current_image = generate_image(ImageType.TOWER, current_image)
...
# save the image path information
if generating_validation:
    np.save('validation_tower_image_path', tower_image_path_information)
else:
    np.save('tower_image_path', tower_image_path_information)
